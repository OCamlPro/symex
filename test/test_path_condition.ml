...
let test_print_pc_one () : unit =
  let pc =
    ...
    |> Symex.Path_condition.add_checked_sat_condition
         (eq (i32_sym "x0") (i32_const 42l))
    |> print
  in
  Alcotest.(check string)
    "same string"
...
let test_slice_on_symbol_transitive () =
  let slice =
    ...
    Symex.Path_condition.empty
    |> Symex.Path_condition.add_checked_sat_condition
         (eq (Smtml.Typed.Bitv32.symbol x0) y0)
    |> Symex.Path_condition.add_checked_sat_condition (eq y0 z0)
    |> Symex.Path_condition.slice_on_symbol x0
  in
  Alcotest.(check int)
...
(* `slice_on_new_condition` *)

let test_slice_on_condition_basic () =
  let slice =
    let c1 = eq (i32_sym "x0") (i32_const 10l) in
    Symex.Path_condition.empty
    |> Symex.Path_condition.add_checked_sat_condition c1
    |> Symex.Path_condition.slice_on_new_condition c1
  in
...
let test_slice_on_condition_transitive () =
  let slice =
    ...
    Symex.Path_condition.empty
    |> Symex.Path_condition.add_checked_sat_condition c1
    |> Symex.Path_condition.add_checked_sat_condition c2
    |> Symex.Path_condition.slice_on_new_condition c1
  in
...
let test_slice_empty () =
  let slices = Symex.Path_condition.empty |> Symex.Path_condition.to_list in
...
let test_slice_independent () =
  let slices =
    Symex.Path_condition.empty
    |> Symex.Path_condition.add_checked_sat_condition
         (eq (i32_sym "x0") (i32_const 1l))
    |> Symex.Path_condition.add_checked_sat_condition
         (eq (i32_sym "y0") (i32_const 2l))
    |> Symex.Path_condition.to_list
  in
